...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, searchVal: ''}
  ...
  getFilteredApps = searchFilterApps => {
    const {activeTabId} = this.state
    const filteredApps = searchFilterApps.filter(
  ...
  getSearchFilteredApps = () => {
    ...
    const searchResults = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchVal.toLowerCase()),
    )
  ...
  render() {
    const {activeTabId, searchVal} = this.state
    const searchFilterApps = this.getSearchFilteredApps()
    const filteredApps = this.getFilteredApps(searchFilterApps)
